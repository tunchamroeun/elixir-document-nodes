...
> extends BaseVisitor<TRawConfig, TPluginConfig> {
  ...
  constructor(
    ...
    this._onExecutableDocumentNode = (
      rawConfig as any
  ...
  protected _transformFragments(fragmentNames: Array<string>): string[] {
    return fragmentNames.map((document) =>
  ...
  protected _gql(
    ...
    if (this.config.documentMode === DocumentMode.documentNodeImportFragments) {
      ...
      const fragmentDependencyNames = new Set(
        fragmentNames
          .map((name) => this.fragmentsGraph.dependenciesOf(name))
          .flatMap((item) => item)
      );
      ...
      return `{${metaString}"kind":"${
        Kind.DOCUMENT
      }","definitions":${jsonStringify(definitions)}}`;
    }
    ...
    if (this.config.documentMode === DocumentMode.string) {
      ...
      if (
        ...
        const meta = this._getGraphQLCodegenMetadata(
          node,
          gqlTag([doc]).definitions
        );
    ...
    while (queue.length) {
      ...
      if (
        ...
        selection.directives?.some(
          (d: DirectiveNode) => d.name.value === "defer"
        )
      ) {
  ...
  protected _generateImport(
    ...
    const typeImport =
      isTypeImport && this.config.useTypeImports ? "import type" : "import";
  ...
  public getImports(options: { excludeFragments?: boolean } = {}): string[] {
      ...
      case DocumentMode.external: {
        if (this._collectedOperations.length > 0) {
          if (
            ...
            let documentPath = `./${this.clearExtension(
              basename(this._documents[0].location || "")
            )}`;
    ...
    if (!excludeFragments) {
      const deduplicatedImports = Object.values(
          ...
          (fragmentImports): ImportDeclaration<FragmentImport> => ({
            ...
            importSource: {
              ...
              identifiers: unique(
                flatten(
                  fragmentImports.map(
                    (fi) => fi?.importSource?.identifiers || []
                  )
                ),
        ...
        .filter(
          (fragmentImport) =>
            fragmentImport.outputPath !== fragmentImport.importSource.path
        );
